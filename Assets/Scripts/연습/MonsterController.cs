    ...
    IEnumerator moveOrder(GameObject[] monsterList)
    {
        ...
        {   
              switch(monster.name){
                    ...
                    case "RushPig(Clone)":
                    isPlayerAround = monster.GetComponent<RushPig>().isPlayerAround();
                    if(!isPlayerAround) monster.GetComponent<crossMovement>().pigMovePattern(3);
                    break;

                    case "FirePig(Clone)":
                    isPlayerAround = monster.GetComponent<FirePig>().firePigMovePattern();
                    if (!isPlayerAround) monster.GetComponent<crossMovement>().pigMovePattern(2);
